...
public class TesteSimplificador {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            ControleMotores.InicializaMotores(new FileInputStream("TestesTxt/TesteTraducaoSimplificador.txt"));
        } catch (IOException e) {
        ...
        for (List<Token> l: Simplificador.SimplificarCodigo(Memoria.TokensReclassificados)) {

            if(l.isEmpty()) {
                Geral.PrintHighlight("\t[Removido]");
            }
            else {
                if(l.get(0).Tipo != Token.IDENTIFICADOR) {
                    System.out.print("\t");
